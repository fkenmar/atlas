...
**Determinism → prompt-cache (2026-06-22).** The prerequisite for prompt-cache reuse is a byte-stable prefix. **Proven, free:** `atlas <repo> --budget 2048` is **byte-identical across runs** (cache-stable); Aider's `--show-repo-map` **differs run-to-run** (confirms [aider#1874](https://github.com/Aider-AI/aider/issues/1874) — different files selected each run, e.g. `src/_pytest/nodes.py`/`_code/source.py` present in one run, absent the next), so a re-injected Aider map **busts** the prompt cache. **Economics (Anthropic, published):** cache reads = 0.1× input price → a stable atlas map re-reads at ~10% cost across turns/sessions within the cache TTL, while a non-deterministic Aider map re-pays full input each turn. **Direct `claude -p` cache-token A/B: inconclusive** — claude -p's own ~31k-token system-prompt/tools cache (cache_create 14,595 + cache_read 16,862, *identical across separate invocations*) dominates the accounting, so a ~2k injected map prefix isn't separable. The win therefore rests on the determinism proof + published cache pricing, not a fresh token A/B. Net: atlas's determinism makes a committed/injected map a cache-stable prefix; Aider's non-determinism does not. (Task #4.)
