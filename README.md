...
**Status: pre-alpha (M0 — Foundation).** The naive end-to-end pipeline works for Python: `repomap <path>` emits a full unranked map (pytest's 92k LOC → mapped in 0.56 s cold). Ranking, budgeting, and the TS/JS + Rust grammars land in M1. See [STATUS.md](STATUS.md) for the live board and [docs/PRD.md](docs/PRD.md) for the full spec.

repomap parses a repository with tree-sitter and emits a compressed structural map — every function signature, type definition, exported symbol, and import edge, with zero function bodies — packed into a token budget (default 2,048) by personalized PageRank over the import/reference graph. A 50k-LOC repo becomes a ~2k-token summary an agent can hold entirely in context, so it stops burning turns on exploration.
...
$ repomap serve --mcp                # MCP server: get_map, get_symbol
$ repomap diff HEAD~5                # API-surface diff between revisions
```
...
**Preliminary with-map probe (same day, unofficial):** injecting even the naive unbudgeted map cut turns **41–43%** on both tasks — but raised total tokens, because the ~81k-token naive map is re-read from cache every turn (92% of the bill). Navigation value proven; converting it into token savings is exactly what M1's ranking + budgeting stage is for. The official with-map comparison lands there. Releases are blocked on this section being current.
