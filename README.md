...
In our benchmark, agents given an atlas map answered structural questions about a codebase using **~30% fewer tokens at identical accuracy** (20/20 correct *with* and *without* the map) — and in **a third fewer turns**. On open-ended edit tasks the map cuts turns too, though token savings there vary by task.
